"""
Moduł głównej aplikacji GUI. Zoptymalizowany pod kątem responsywności i asynchronicznego ładowania widoków.
"""
...
class SimpleBudgetApp(ctk.CTk):
    def __init__(self):
        ...
        ctk.CTkButton(top, text="Statystyki", fg_color="transparent", hover_color="#111217", font=("Segoe UI", 14, "bold"), command=lambda: self.switch_view("Statystyki")).pack(side="right", padx=(4, 8))
        ctk.CTkButton(top, text="Strona główna", fg_color="transparent", hover_color="#111217", font=("Segoe UI", 14, "bold"), command=lambda: self.switch_view("Main")).pack(side="right", padx=(4, 4))
    ...
    def render(self):
        ...
        if self.view_var == "Main":
            ...
            ctk.CTkLabel(body_left, text="Łącznie na wszystkich kontach", text_color="#94A3B8", font=("Segoe UI", 13)).pack(anchor="w", padx=8, pady=(4, 0))
            ...
            for item in self.db.transactions():
                ...
                # Nowa, zhierarchizowana struktura kafelka transakcji [Źródło: Standardy wizualizacji układów UI]
                
                # Górny wiersz: Tytuł (Notatka lub Kategoria) oraz Kwota transakcji
                top_row = ctk.CTkFrame(row, fg_color="transparent")
                top_row.pack(fill="x", padx=14, pady=(12, 2))
                
                # Jeśli użytkownik nie wpisał notatki, tytułem staje się automatycznie tag transakcji
                ...
                # Dolny wiersz: Szare metadane techniczne oraz przycisk akcji
                ...
                ctk.CTkButton(bottom_row, text="Ponów", width=60, height=24, fg_color="#3A3A3C", hover_color="#48484A", text_color="#FFFFFF", font=("Segoe UI", 11, "bold"), corner_radius=6, command=lambda t=item: self.repeat_transaction(t)).pack(side="right")
            ...
            if stats:
                ...
                fig = Figure(figsize=(10, 4), facecolor='#1B1D29')
                ax = fig.add_subplot(111)
                ax.set_facecolor('#1B1D29')
                bars = ax.barh(tags, spent, color='#FF3B30', height=0.6)
                ax.set_title("Wydatki według kategorii", color="white", fontsize=12)
            ...
            for item in stats:
                card = ctk.CTkFrame(stat_scroll, fg_color="#252837", corner_radius=10)
                card.pack(fill="x", padx=20, pady=6)
                ctk.CTkLabel(card, text=item['tag'], text_color="#F8FAFC", font=("Segoe UI", 14, "bold")).pack(side="left", padx=15, pady=15)
                ctk.CTkLabel(card, text=f"Wydano: {item['spent']:.2f} zł   •   Wpłaty: {item['income']:.2f} zł", text_color="#CBD5E1", font=("Segoe UI", 12)).pack(side="right", padx=15)

            if not stats:
                ctk.CTkLabel(stat_scroll, text="Brak danych — dodaj pierwszy wpis", text_color="#CBD5E1", font=("Segoe UI", 13)).pack(pady=20)
